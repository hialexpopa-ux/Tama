...
Chaque appareil garde **son propre pet**, persisté **localement** par le **`store`**
(`localStorage`). C'est un choix **assumé** (Alex, 2026-07-02) : **pas de synchronisation
entre appareils**. Ce que les trois portes partagent, c'est l'**app et l'art** (même URL,
mêmes PNG), pas la vie du pet. → **Firebase / phase 2 abandonnés** (voir `HANDOFF.md`).
...
vérifiée** — specs **PWA / service worker**, **versions de libs**,
...
- **Servir la PWA en local** (service worker impossible en `file://`) :
  `npm start` (= `node tools/serve.mjs`), `npx serve .` ou `python -m http.server 8000`,
...
**App locale, une seule phase** (la sync/phase 2 est abandonnée) : moteur `tama.js` +
`constants.js` → `store.js` (local) → UI + `game.js` → art via manifeste → PWA
(`manifest.webmanifest` + `sw.js`) → Andy lanceur.
...
PNG d'Alex, et vivre une vraie partie en `MODE = 'official'`.
